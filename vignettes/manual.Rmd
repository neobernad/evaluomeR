...
calculus: *fpc* [@fpc2018], *cluster* [@cluster2018], *corrplot*
...
Bioconductor package *SummarizedExperiment* [@summarizedExperiment] for
input/output data.
...
We provide the `correlations` function to evaluate the correlations among the
...
`SummarizedExperiment` that contains an assay with the information shown in
the plot:
...
The `qualityRangeData` object returned by `qualityRange` is list of
`SummarizedExperiment`, whose size is `diff(k.range)+1`. In the example shown
above, the size of `qualityRangeData` is 3, since the array length would
contain the dataframes from `k=4` to `k=6`.
...
analysis functions: `correlations`, `stability`, `stabilityRange`, `quality`
and `qualityRange`.
