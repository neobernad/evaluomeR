...
# Stability [2,6]
dataFrame <- stabilityRange(data=inputData, k.range=c(2,6), bs=100, getImages = TRUE, seed=13606)
dataStability = assay(dataFrame, 1)
csvPath = paste0(outputDir, "/stability.csv")
write.csv(dataStability, csvPath, row.names = FALSE)

# Quality [2,6]
dataFrame <- qualityRange(data=inputData, k.range=c(2,6), getImages = TRUE, seed=13606)
for (i in 2:6) {
